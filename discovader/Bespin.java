 ...
 */
public class Bespin extends DiscoRobot
{
	...
	public void run() 
	{
		...
		setAllColors(new Color(204, 153, 153));				//sets the colours of the robot
	...
	void antiGravMove() 
	{
	    ...
	    GravPoint p;
		Enemy en;
    	Iterator<Enemy> it = targets.values().iterator();
	    ...
	    xforce += 5000/Math.pow(getRange(getX(), getY(), getBattleFieldWidth(), getY()), 2.5);
	    xforce -= 5000/Math.pow(getRange(getX(), getY(), 0, getY()), 2.5);
	    yforce += 5000/Math.pow(getRange(getX(), getY(), getX(), getBattleFieldHeight()), 2.5);
	    yforce -= 5000/Math.pow(getRange(getX(), getY(), getX(), 0), 2.5);
	...
	void OLDdoGun() 
	{
		...
		Point p=target.guessPosition(time, getTime());
